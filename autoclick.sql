...
BEGIN
DECLARE $click_mult,$current_cash,$current_time,$current_spent,$current_mult,$current_thresh,$current_tmult,$autoclick_cnt,$autoclick_cost,$purchase_diff,$purchase_time,$system,$time_case,$total_cash,$total_spent,$total_tmult,$total_time,$total_mult,$wallet LONGTEXT;
...
WHERE
	...
	-- FAILURE --
when $autoclick_cnt < 1 or $autoclick_cnt is NULL THEN 											
                SET $current_time = (select timestampdiff(second,created_at, current_timestamp()) from click WHERE active=1),
					$current_mult = (select mult from click where active=1),
                    $total_cash = (select sum($total_tmult-$total_spent)),
                    $total_spent = (select sum(cspent) from click WHERE id!=1),
